"""
Encode 3 frames per clip (t=2, 5, 8) with CLIP ViT-B/32, mean-pool to one
feature per clip, then run the same subject-disjoint linear probe.

Tests whether temporal context recovers engagement signal that single-frame
features miss.

Inputs:
  frames_full/{Train,Validation,Test}/<clip>.jpg              (t=5 from script 09)
  frames_full_multi/{Train,Validation,Test}/<clip>_t{2,8}.jpg (from script 12)

Output:
  features/clip_vitb32_multiframe_features.npz
  results/linear_probe/probe_results_multiframe.json
"""
...
MULTI_DIR = os.path.join(BASE_DIR, "frames_full_multi")
FEAT_FILE = os.path.join(BASE_DIR, "features", "clip_vitb32_multiframe_features.npz")
...
def encode_features():
    if os.path.exists(FEAT_FILE):
        print(f"Multiframe features exist at {FEAT_FILE}, skipping encoding.")
        return
    ...
    with open(MANIFEST) as f:
        ...
        for r in reader:
            ...
            t5 = r["frame_path"]
            t2 = os.path.join(MULTI_DIR, r["split"], f"{base}_t2.jpg")
            t8 = os.path.join(MULTI_DIR, r["split"], f"{base}_t8.jpg")
            if not (os.path.exists(t5) and os.path.exists(t2) and os.path.exists(t8)):
                continue
            rows.append({**r, "t2": t2, "t5": t5, "t8": t8})
    print(f"Clips with all 3 frames: {len(rows)}")
    ...
    with torch.no_grad():
        for i in range(0, len(rows), BATCH):
            ...
            # Encode 3 frames per clip, mean-pool, re-normalize
            imgs2 = torch.stack([preprocess(Image.open(r["t2"]).convert("RGB")) for r in batch]).to(device)
            imgs5 = torch.stack([preprocess(Image.open(r["t5"]).convert("RGB")) for r in batch]).to(device)
            imgs8 = torch.stack([preprocess(Image.open(r["t8"]).convert("RGB")) for r in batch]).to(device)
            f2 = model.encode_image(imgs2); f2 = f2 / f2.norm(dim=-1, keepdim=True)
            f5 = model.encode_image(imgs5); f5 = f5 / f5.norm(dim=-1, keepdim=True)
            f8 = model.encode_image(imgs8); f8 = f8 / f8.norm(dim=-1, keepdim=True)
            mean = (f2 + f5 + f8) / 3.0
            mean = mean / mean.norm(dim=-1, keepdim=True)
            feats[i:i + len(batch)] = mean.cpu().numpy()
            if (i // BATCH) % 10 == 0:
